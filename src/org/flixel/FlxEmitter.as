package org.flixel
{
	...
	public class FlxEmitter extends FlxCore
	{
		...
		public function emit():void
		{
			...
			if (s.body)
			{
				s.body.SetLinearVelocity(
					new b2Vec2(FlxG.random() * (maxVelocity.x - minVelocity.x) + minVelocity.x,
										  FlxG.random() * (maxVelocity.x - minVelocity.x) + minVelocity.y));
				s.body.SetAngularVelocity( FlxG.random() * (maxRotation - minRotation) + minRotation);
				s.body.SetLinearDamping(drag);
			}
			_particle++;
